...
function separate_traffic() {
  const lastCarIndex = traffic.length - 1;
  const secondLastCarIndex = traffic.length - 2;

  if (lastCarIndex >= 0 && secondLastCarIndex >= 0) {
    const lastCar = traffic[lastCarIndex];
    const secondLastCar = traffic[secondLastCarIndex];

    if (lastCar.x === secondLastCar.x) {
      lastCar.x -= 1;
      if (lastCar.x < 0) {
        lastCar.x = 2;
      }
...
function generate_cars(N) {
  const cars = [];
  for (let i = 0; i <= N; i++) {
    cars.push(new Car(road.getLaneCenter(1), 100, 30, 70, "AI", 5));
  }
  return cars;
}
